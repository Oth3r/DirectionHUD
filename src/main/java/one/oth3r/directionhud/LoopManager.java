...
public class LoopManager {
    ...
    public static void tick() {
        ...
        if (tickS >= 20) {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                //PARTICLES
                Vector pVec = new Loc(player).getVec3d(player).add(new Vector(0, 1, 0));
                if (Destination.get(player).hasXYZ()) {
                    ...
                    if (PlayerData.get.dest.setting.particle.dest(player)) {
                        Vector particlePos = destVec.add(new Vector(0, 3, 0));
                        double spacing = 1;
                        Vector segment = destVec.add(new Vector(0, -3, 0)).subtract(particlePos).normalize().multiply(spacing);
                ...
                if (trackingP != null && PlayerData.get.dest.setting.track(trackingP)) {
                    ...
                    Vector trackingVec = new Loc(trackingP).getVec3d(trackingP);
